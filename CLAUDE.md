...
src/
├── app/
│   ├── layout.tsx          # Root layout (LTR, Inter font, providers)
...
├── hooks/
│   └── use-formatted-date.ts # date-fns formatting hook
├── lib/
│   ├── db.ts               # Drizzle + libSQL client (lazy proxy)
│   ├── platform.ts         # getPlatformContext()
│   ├── cn.ts               # Tailwind class merge
│   ├── format.ts           # number, price, date, phone, card utils
│   └── validations/
│       └── index.ts        # Zod schemas: email, phone, card, forms
├── db/
...
This template is English-first (`lang="en"`, `dir="ltr"`). It does not ship a heavy i18n library.

If a project needs another locale or full bilingual support, add a tiny helper:
...
export function t(en: string, other: string): string {
  return getLocale() === "en" ? en : other
}
```

For RTL languages, switch `dir="ltr"` → `dir="rtl"` in `layout.tsx` and use Tailwind logical properties (`ms-`, `me-`, `ps-`, `pe-`, `text-start`, `text-end`, `start-0`, `end-0`) so spacing flips correctly.
...
Ready-to-use helpers. Agent should use these instead of reinventing:

| Function | What it does | Example |
|----------|-------------|---------|
| `formatNumber(n, locale?)` | `50000` → `"50,000"` | Counters, stats |
| `formatPrice(amount, currency?, locale?)` | `1999` → `"$1,999.00"` | Price display |
| `formatDate(d)` | `"April 30, 2026"` | Blog posts, orders |
| `formatDateTime(d)` | `"April 30, 2026 at 14:30"` | Order details |
| `formatDateShort(d)` | `"2026/04/30"` | Compact tables |
| `formatRelativeTime(d)` | `"3 minutes ago"` | Comments, chat |
| `validatePhone(p)` | Validates US phone (10 digits) | Phone input |
| `formatPhone(p)` | `"5551234567"` → `"(555) 123-4567"` | Display |
| `validateCreditCard(c)` | Luhn check on 13–19 digit card | Payment forms |
| `formatCreditCard(c)` | `"4111111111111111"` → `"4111 1111 1111 1111"` | Display |
...
Pre-built schemas with English error messages:

| Schema | Use for |
|--------|---------|
| `phoneSchema` | Phone input (validates US phone) |
| `creditCardSchema` | Card input (Luhn + 13–19 digits) |
...
| `signupSchema` | Signup form (name + email + password) |
...
- `NEXT_PUBLIC_LOCALE` — defaults to `"en"`
- `PROJECT_ID`
- `VERSION`
- `RUNTIME_LANE`
